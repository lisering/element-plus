...
import { TypeComponentsMap } from '@hicor-ui/utils'
...
describe('Alert.vue', () => {
  ...
  test('description', () => {
    ...
    expect(wrapper.find('.hc-alert__description').text()).toEqual(AXIOM)
  })

  test('theme', () => {
    const wrapper = mount(() => <Alert title={'test'} effect={'dark'} />)
    expect(wrapper.find('.hc-alert').classes()).toContain('is-dark')
  })
  ...
  test('close', async () => {
    ...
    const closeBtn = wrapper.find('.hc-alert__close-btn')
