...
import type { TabsPaneContext } from '@hicor-ui/components/tabs'
...
describe('Tabs.vue', () => {
  ...
  test('dynamic', async () => {
    ...
    let navItemsWrapper = navWrapper.findAll('.hc-tabs__item')
    ...
    navItemsWrapper = navWrapper.findAll('.hc-tabs__item')
  ...
  test('editable', async () => {
    ...
    await wrapper.find('.hc-tabs__new-tab').trigger('click')
  ...
  test('tab order', async () => {
    ...
    const items = wrapper.findAll('.hc-tabs__item')
  ...
  test('tab-position', async () => {
    ...
    expect(tabsWrapper.find('.hc-tabs__active-bar').classes('is-left')).toBe(
      true
    )
    expect(tabsWrapper.find('.hc-tabs__item').classes('is-left')).toBe(true)
  })
  ...
  test('stretch', async () => {
    ...
    expect(tabsWrapper.find('.hc-tabs__nav').classes('is-stretch')).toBe(true)

    tabPosition.value = 'left'
    await nextTick()

    expect(tabsWrapper.find('.hc-tabs__nav').classes('is-stretch')).toBe(false)
  })
  ...
  test('tab active bar offset', async () => {
    ...
    await nextTick()
    expect(tabsWrapper.find('.hc-tabs__active-bar').attributes().style).toMatch(
      'translateX(300px)'
    )
  ...
  test('should work with lazy', async () => {
    ...
    expect(wrapper.findAll('.hc-tab-pane').length).toBe(3)

    await navItemsWrapper[3].trigger('click')

    expect(wrapper.findAll('.hc-tab-pane').length).toBe(4)
  })
