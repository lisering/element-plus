...
import { IMAGE_SUCCESS } from '@hicor-ui/test-utils/mock'
...
describe('<image-viewer />', () => {
  test('big image preview', async () => {
    ...
    await doubleWait()
    const viewer = wrapper.find('.hc-image-viewer__wrapper')
    expect(viewer.exists()).toBe(true)
    await wrapper.find('.hc-image-viewer__close').trigger('click')
  ...
  test('image preview hide-click-on-modal', async () => {
    ...
    const viewer = wrapper.find('.hc-image-viewer__wrapper')
    expect(viewer.exists()).toBe(true)
    await wrapper.find('.hc-image-viewer__mask').trigger('click')
  ...
  test('manually switch image', async () => {
    ...
    const imgList = wrapper.findAll('.hc-image-viewer__img')
